...
class PromoController {
  static async getPromo(req, res, next) {
    ...
    try {
      ...
      const promos = await Fruits.findAll({
        where: {
          userId: decode.id,
        },
      ...
      await Fruits.update({ isRead: true }, { where: { id: promoIds } });
  ...
  static async detailPromo(req, res, next) {
    console.log('[LOG]: Detail Fruits', req.params);
    const { id } = req.params;

    try {
      const notification = await Fruits.findOne({
        where: {
          id,
      ...
      return outputParser.success(req, res, 'Berhasil Mendapatkan Detail Notifikasi', {
        notification,
      });
    } catch (err) {
      return next(err);
  ...
  static async createPromo(req, res, next) {
    ...
    try {
      const notifications = await Fruits.bulkCreate(dummyData);
